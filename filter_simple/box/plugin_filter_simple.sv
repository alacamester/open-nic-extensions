...
Monitor_100G_simple  mon_100g(
         .Clk 		    (cmac_clk[i]),
         .clk_pci       (axil_aclk),
         .Rst 		    (mon_rst[i]),
//-- Filter data Wr
         .DATA_IN 		(mon_data[`getvec(32,i)]),
         .DATA_Wr 		(mon_wr[i]),      
//-- CMAC AXI IN
	     .CMAC_Dav_IN 	(s_axis_cmac_rx_tvalid[i]),
	     .CMAC_Data_IN	(s_axis_cmac_rx_tdata[`getvec(512, i)]),
	     .CMAC_Tkeep_IN (s_axis_cmac_rx_tkeep[`getvec(64, i)]),
	     .CMAC_Eof_IN	(s_axis_cmac_rx_tlast[i]),
//-- CMAC AXI OUT
	     .CMAC_Dav_OUT 	   (m_axis_adap_rx_322mhz_tvalid[i]),
	     .CMAC_Data_OUT    (m_axis_adap_rx_322mhz_tdata[`getvec(512, i)]),
	     .CMAC_Tkeep_OUT   (m_axis_adap_rx_322mhz_tkeep[`getvec(64, i)]),
	     .CMAC_Eof_OUT 	   (m_axis_adap_rx_322mhz_tlast[i])
	);
